...
class RegisterView(APIView):   
    ...
    def post(self, request):
        try:
            # Normalize role to uppercase before validation (choices are uppercase)
            request_data = request.data.copy()
            if 'role' in request_data and isinstance(request_data.get('role'), str):
                request_data['role'] = request_data.get('role').upper()
